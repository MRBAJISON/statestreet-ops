...
export async function getStoreDomain(scope: AnalyticsScope): Promise<StoreDomain> {
  ...
  const transactionResult = await db.execute(sql`
    select
      ...
      (select coalesce(sum(sale.total_value), 0) from customer_credit_sales sale where sale.store_id = ${scope.store.id} and sale.business_date between ${scope.from}::date and ${scope.to}::date) as credit_sales,
      (select coalesce(sum(payment.amount), 0) from customer_credit_sale_payments payment where payment.store_id = ${scope.store.id} and payment.business_date between ${scope.from}::date and ${scope.to}::date) as credit_collections,
      (select coalesce(sum(sale.open_value), 0) from customer_credit_sales sale where sale.store_id = ${scope.store.id} and sale.status in ('open', 'partial')) as open_credit_balance,
  ...
  return {
    ...
    customerTransactions: {
      ...
      creditCollections: Number(row?.credit_collections ?? 0),
      openCreditBalance: Number(row?.open_credit_balance ?? 0),
