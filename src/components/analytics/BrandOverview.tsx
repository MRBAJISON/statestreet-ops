    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <section className="surface min-w-0 overflow-hidden xl:col-span-7">
          ...
          <Table><TableHeader><TableRow><TableHead>Brand</TableHead><TableHead className="text-right">Health</TableHead><TableHead className="text-right">Momentum</TableHead><TableHead className="text-right">Sentiment</TableHead><TableHead className="text-right">Rating</TableHead></TableRow></TableHeader><TableBody>{domain.brands.length ? domain.brands.map((brand) => <TableRow key={brand.id}><TableCell className="font-medium">{brand.name}</TableCell><TableCell className="text-right">{formatPercent(brand.health)}</TableCell><TableCell className="text-right">{formatPercent(brand.momentum)}</TableCell><TableCell className="text-right">{formatPercent(brand.positiveSentiment)}</TableCell><TableCell className="text-right">{brand.googleRating?.toFixed(2) ?? 'No data'}</TableCell></TableRow>) : <EmptyTableRow colSpan={5} message="No brand-health assessments have been submitted" />}</TableBody></Table>
        </section>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <section className="surface min-w-0 overflow-hidden xl:col-span-8">
          ...
          <Table><TableHeader><TableRow><TableHead>Brand</TableHead><TableHead className="text-right">Awareness</TableHead><TableHead className="text-right">Consideration</TableHead><TableHead className="text-right">Preference</TableHead><TableHead className="text-right">Satisfaction</TableHead><TableHead className="text-right">Loyalty</TableHead><TableHead className="text-right">Advocacy</TableHead></TableRow></TableHeader><TableBody>{domain.brands.length ? domain.brands.map((brand) => <TableRow key={brand.id}><TableCell className="font-medium">{brand.name}</TableCell><TableCell className="text-right">{formatPercent(brand.awareness)}</TableCell><TableCell className="text-right">{formatPercent(brand.consideration)}</TableCell><TableCell className="text-right">{formatPercent(brand.preference)}</TableCell><TableCell className="text-right">{formatPercent(brand.satisfaction)}</TableCell><TableCell className="text-right">{formatPercent(brand.loyalty)}</TableCell><TableCell className="text-right">{formatPercent(brand.advocacy)}</TableCell></TableRow>) : <EmptyTableRow colSpan={7} message="No brand-equity detail has been submitted" />}</TableBody></Table>
        </section>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <section className="surface p-5"><SectionHeading title="Risks" description="Current brand risks requiring mitigation" />{domain.risks.length ? <div className="mt-4 divide-y">{domain.risks.map((item, index) => <div key={`${item.text}-${index}`} className="flex gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-3/10 text-destructive"><ShieldAlert className="size-4" /></span><span className="min-w-0 flex-1 text-sm leading-5">{item.text}</span><StatusBadge value={item.tag} /></div>)}</div> : <EmptyPanel message="No brand risks have been submitted" />}</section>
        <section className="surface p-5"><SectionHeading title="Opportunities" description="Brand-growth opportunities worth testing" />{domain.opportunities.length ? <div className="mt-4 divide-y">{domain.opportunities.map((item, index) => <div key={`${item.text}-${index}`} className="flex gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary"><Lightbulb className="size-4" /></span><span className="min-w-0 flex-1 text-sm leading-5">{item.text}</span><StatusBadge value={item.tag} /></div>)}</div> : <EmptyPanel message="No brand opportunities have been submitted" />}</section>
      </div>
      ...
      <section className="surface p-5">
        ...
        {domain.attention.length ? <div className="mt-4 grid gap-x-6 md:grid-cols-2">{domain.attention.map((item) => <div key={item.id} className="flex gap-3 border-b py-3 first:pt-0 md:[&:nth-last-child(-n+2)]:border-b-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-3/10 text-destructive"><ShieldAlert className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.title}</span><span className="block truncate text-xs text-muted-foreground">{item.ownerName} / {item.dueDate ?? 'No due date'}</span></span><StatusBadge value={item.priority} /></div>)}</div> : <EmptyPanel message="No brand escalations require CEO attention" />}
      </section>
    </div>
  );
}
