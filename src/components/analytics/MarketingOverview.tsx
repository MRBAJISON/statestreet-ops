    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <section className="surface p-5">
          ...
          {domain.feedbackDetail.length ? <div className="mt-4 divide-y">{domain.feedbackDetail.slice(0, 9).map((item) => <div key={item.id} className="flex gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-5/10 text-chart-5"><MessageSquareText className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium capitalize">{item.type} / {item.storeName ?? 'Group'}</span><span className="block truncate text-xs text-muted-foreground">{item.detail}</span></span><span className="text-xs font-medium text-muted-foreground">{item.frequency ?? item.source}</span></div>)}</div> : <EmptyPanel message="No customer feedback has been submitted" />}
        </section>
      </div>
