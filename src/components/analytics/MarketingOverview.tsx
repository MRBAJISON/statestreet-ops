    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <section className="surface min-w-0 overflow-hidden">
          ...
          <Table><TableHeader><TableRow><TableHead>Activity</TableHead><TableHead className="text-right">Contacted</TableHead><TableHead className="text-right">Responses</TableHead><TableHead className="text-right">Appointments</TableHead><TableHead className="text-right">Revenue</TableHead></TableRow></TableHeader><TableBody>{domain.clienteling.map((activity) => <TableRow key={activity.type}><TableCell className="font-medium capitalize">{activity.type}</TableCell><TableCell className="text-right">{activity.contacted}</TableCell><TableCell className="text-right">{activity.responses}</TableCell><TableCell className="text-right">{activity.appointments}</TableCell><TableCell className="text-right font-medium">{formatCurrency(activity.revenue, meta.currency)}</TableCell></TableRow>)}</TableBody></Table>
        </section>
        <section className="surface p-5">
          <SectionHeading title="Customer Experience" description="Feedback themes and recurring customer signals" />
          <div className="mt-4 divide-y">{domain.feedbackDetail.slice(0, 9).map((item) => <div key={item.id} className="flex gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-5/10 text-chart-5"><MessageSquareText className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium capitalize">{item.type} / {item.storeName ?? 'Group'}</span><span className="block truncate text-xs text-muted-foreground">{item.detail}</span></span><span className="text-xs font-medium text-muted-foreground">{item.frequency ?? item.source}</span></div>)}</div>
        </section>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <section className="surface p-5 xl:col-span-8">
          ...
          <div className="mt-4 divide-y">{domain.actions.slice(0, 12).map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary"><Megaphone className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.title}</span><span className="block truncate text-xs text-muted-foreground">{item.ownerName} / {item.storeName ?? 'Group'} / {item.dueDate ?? 'No due date'}</span></span><StatusBadge value={item.priority} /><StatusBadge value={item.status} /></div>)}</div>
        </section>
      </div>
    </div>
  );
