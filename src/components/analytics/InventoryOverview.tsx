    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <section className="surface min-w-0 overflow-hidden xl:col-span-8">
          ...
          <Table><TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Store</TableHead><TableHead className="text-right">Current</TableHead><TableHead className="text-right">Reorder</TableHead><TableHead>Urgency</TableHead></TableRow></TableHeader><TableBody>{domain.replenishmentLines.length ? domain.replenishmentLines.slice(0, 14).map((item) => <TableRow key={item.id}><TableCell><span className="block max-w-56 truncate font-medium">{item.productName}</span><span className="text-xs text-muted-foreground">{item.sku}</span></TableCell><TableCell>{item.storeName}</TableCell><TableCell className="text-right">{item.currentStock}</TableCell><TableCell className="text-right font-medium">{item.reorderQuantity}</TableCell><TableCell><StatusBadge value={item.urgency} /></TableCell></TableRow>) : <EmptyTableRow colSpan={5} message="No replenishment lines require attention" />}</TableBody></Table>
        </section>
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        <section className="surface min-w-0 overflow-hidden xl:col-span-8">
          ...
          <Table><TableHeader><TableRow><TableHead>Product</TableHead><TableHead>Store</TableHead><TableHead>Action</TableHead><TableHead>Justification</TableHead><TableHead className="text-right">Value</TableHead><TableHead>Status</TableHead></TableRow></TableHeader><TableBody>{domain.dispositions.length ? domain.dispositions.slice(0, 14).map((item) => <TableRow key={item.id}><TableCell><span className="block max-w-48 truncate font-medium">{item.productName}</span><span className="text-xs text-muted-foreground">{item.categoryName}</span></TableCell><TableCell>{item.storeName}</TableCell><TableCell className="capitalize">{item.action}</TableCell><TableCell className="max-w-56 truncate text-muted-foreground">{item.justification}</TableCell><TableCell className="text-right">{formatCurrency(item.value, meta.currency)}</TableCell><TableCell><StatusBadge value={item.status} /></TableCell></TableRow>) : <EmptyTableRow colSpan={6} message="No dead-stock actions recorded" />}</TableBody></Table>
        </section>
        <section className="surface min-w-0 p-5 xl:col-span-4">
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <section className="surface p-5 xl:col-span-8">
          ...
          {domain.receiptIssues.length ? <div className="mt-4 divide-y">{domain.receiptIssues.slice(0, 12).map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-3/10 text-destructive"><PackageCheck className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.supplierName} / {item.storeName}</span><span className="block truncate text-xs text-muted-foreground">{item.date} / {item.discrepancy}</span></span><StatusBadge value={item.condition} /></div>)}</div> : <EmptyPanel message="No goods-receipt issues recorded" />}
        </section>
      </div>
      ...
      <section className="surface p-5">
        <Tabs defaultValue="receipts">
          ...
          <TabsContent value="receipts" className="mt-4 divide-y">{domain.receipts.length ? domain.receipts.map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 items-center justify-center rounded-md bg-primary/10 text-primary"><PackageCheck className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.supplierName} / {item.storeName}</span><span className="text-xs text-muted-foreground">{item.date} / {item.units} units / {item.poNumber ?? 'No PO'}</span></span><span className="text-sm font-semibold">{formatCurrency(item.value, meta.currency)}</span></div>) : <EmptyPanel message="No goods receipts recorded for this period" />}</TabsContent>
          <TabsContent value="transfers" className="mt-4 divide-y">{domain.transfers.length ? domain.transfers.map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 items-center justify-center rounded-md bg-chart-4/10 text-chart-4"><Truck className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.fromStore} to {item.toStore}</span><span className="text-xs text-muted-foreground">{item.date} / {item.units} units</span></span><StatusBadge value={item.status} /></div>) : <EmptyPanel message="No stock transfers recorded for this period" />}</TabsContent>
          <TabsContent value="replenishments" className="mt-4 divide-y">{domain.replenishments.length ? domain.replenishments.map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 items-center justify-center rounded-md bg-chart-2/12 text-amber-800"><SendToBack className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.storeName}</span><span className="text-xs text-muted-foreground">{item.date} / {item.lines} lines / {item.units} units</span></span><StatusBadge value={item.status} /></div>) : <EmptyPanel message="No replenishment requests recorded for this period" />}</TabsContent>
        </Tabs>
      </section>
    </div>
  );
