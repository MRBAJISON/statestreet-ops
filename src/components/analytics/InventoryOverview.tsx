    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <section className="surface min-w-0 p-5 xl:col-span-7">
          <SectionHeading title="Inventory Value Trend" description="Goods-receipt value recorded over the selected period" />
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        ...
        <section className="surface p-5 xl:col-span-8">
          ...
          <div className="mt-4 divide-y">{domain.receiptIssues.slice(0, 12).map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-3/10 text-destructive"><PackageCheck className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.supplierName} / {item.storeName}</span><span className="block truncate text-xs text-muted-foreground">{item.date} / {item.discrepancy}</span></span><StatusBadge value={item.condition} /></div>)}</div>
        </section>
      </div>
      ...
      <section className="surface p-5">
        <Tabs defaultValue="receipts">
          ...
          <TabsContent value="receipts" className="mt-4 divide-y">{domain.receipts.map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 items-center justify-center rounded-md bg-primary/10 text-primary"><PackageCheck className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.supplierName} / {item.storeName}</span><span className="text-xs text-muted-foreground">{item.date} / {item.units} units / {item.poNumber ?? 'No PO'}</span></span><span className="text-sm font-semibold">{formatCurrency(item.value, meta.currency)}</span></div>)}</TabsContent>
          <TabsContent value="transfers" className="mt-4 divide-y">{domain.transfers.map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 items-center justify-center rounded-md bg-chart-4/10 text-chart-4"><Truck className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.fromStore} to {item.toStore}</span><span className="text-xs text-muted-foreground">{item.date} / {item.units} units</span></span><StatusBadge value={item.status} /></div>)}</TabsContent>
          <TabsContent value="replenishments" className="mt-4 divide-y">{domain.replenishments.map((item) => <div key={item.id} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 items-center justify-center rounded-md bg-chart-2/12 text-amber-800"><SendToBack className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.storeName}</span><span className="text-xs text-muted-foreground">{item.date} / {item.lines} lines / {item.units} units</span></span><StatusBadge value={item.status} /></div>)}</TabsContent>
        </Tabs>
      </section>
    </div>
  );
