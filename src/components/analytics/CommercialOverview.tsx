    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <section className="surface min-w-0 p-5">
          ...
          <HorizontalBarChart data={trading.categories.map((item) => ({ name: item.name, value: item.sellThrough }))} valueFormatter={formatPercent} />
        </section>
        <section className="surface min-w-0 p-5">
      ...
      <section className="surface min-w-0 overflow-hidden">
        ...
        <Table>
          ...
          <TableBody>{domain.productVelocity.slice(0, 18).map((product) => <TableRow key={product.id}><TableCell><span className="block max-w-56 truncate font-medium">{product.name}</span><span className="text-xs text-muted-foreground">{product.sku}</span></TableCell><TableCell><span className="block text-sm">{product.brandName}</span><span className="text-xs text-muted-foreground">{product.categoryName}</span></TableCell><TableCell className="text-right font-medium">{product.unitsSold}</TableCell><TableCell className="text-right">{product.stock}</TableCell><TableCell><StatusBadge value={product.performance ?? product.status ?? 'unrated'} /></TableCell><TableCell className="max-w-72 truncate text-muted-foreground">{product.insight ?? product.campaign ?? 'No insight recorded'}</TableCell></TableRow>)}</TableBody>
        </Table>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <section className="surface p-5">
          ...
          <div className="mt-4 divide-y">{domain.actions.slice(0, 10).map((item) => <div key={item.id} className="flex gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-2/12 text-amber-800"><ShoppingBag className="size-4" /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium">{item.title}</span><span className="block truncate text-xs text-muted-foreground">{item.ownerName} / {item.storeName ?? 'Group'} / {item.dueDate ?? 'No due date'}</span></span><StatusBadge value={item.status} /></div>)}</div>
        </section>
        <section className="surface p-5">
          <SectionHeading title="Manager Voices" description="Direct store-manager observations for the commercial team" />
          <div className="mt-4 divide-y">{domain.managerVoices.slice(0, 10).map((voice) => <div key={voice.reviewId} className="flex gap-3 py-3 first:pt-0 last:pb-0"><span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-chart-5/10 text-chart-5"><UserRoundCheck className="size-4" /></span><span className="min-w-0 flex-1"><span className="block text-sm font-medium">{voice.storeName} / {voice.managerName}</span><span className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{voice.marketingAmplify ?? voice.differentThisWeek ?? voice.firstThreeActions ?? 'No note submitted.'}</span></span></div>)}</div>
        </section>
      </div>
