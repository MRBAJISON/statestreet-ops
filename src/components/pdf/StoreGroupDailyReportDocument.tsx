...
import type { StoreGroupDailyReport } from '@/lib/reporting/store-group-daily-report';

// The daily cluster report: one day, both shops, with a per-store split so the
// combined figure can be read rather than taken on trust. Same visual language as
// the single-store daily report and the weekly/monthly cluster reports.
...
export function StoreGroupDailyReportDocument({
  report,
  currency,
  paymentMethodNames,
  categoryNames,
}: {
  report: StoreGroupDailyReport;
  currency: string;
  ...
  return (
    <Document title={`${report.group.name} daily report ${report.businessDate}`}>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View>
            <Text style={styles.headerTitle}>DAILY CLUSTER REPORT</Text>
          ...
          <View>
            ...
            <Text style={styles.headerMetaValue}>{fullDate(report.businessDate)}</Text>
        ...
        <View style={styles.kpiRow}>
          ...
          <View style={styles.kpiCard}>
            ...
            <Text style={styles.kpiDetail}>Above / below the day&apos;s goal</Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <View style={styles.commentaryBox}>
            <Text style={styles.commentaryHeading}>Day at a Glance</Text>
            <Text style={styles.commentaryText}>
              {leadStore && totals.netRevenue > 0
                ? `${leadStore.storeName} led the day with ${formatMoney(leadStore.netRevenue, currency)}. `
                : ''}
              ...
              {report.leadsCount
                ? `${report.leadsCount} lead${report.leadsCount === 1 ? '' : 's'} captured across the cluster.`
                : 'No leads captured.'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>2. SALES BREAKDOWN (COMBINED)</Text>
          <View style={styles.card}>
        ...
        {report.categories.length ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>3. SALES BY CATEGORY (COMBINED)</Text>
            <View style={styles.card}>
              <View style={styles.tableHead}>
        ...
        {report.productsSold.length ? (
          <View style={styles.section}>
            ...
            <View style={styles.card}>
              {report.productsSold.slice(0, 25).map((product, index) => (
                <View key={`${product.storeName}-${product.name}-${index}`} style={styles.bullet}>
                  <Text style={styles.bulletDot}>•</Text>
                  <Text style={styles.bulletText}>
                    {product.name}
                    <Text style={{ color: COLORS.muted }}> — {product.storeName}</Text>
                  </Text>
                  <Text style={{ color: COLORS.muted }}>{product.unitsSold}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            {report.customerRequests.length ? (
              report.customerRequests.map((request, index) => (
              ...
              <Text style={styles.rowLabel}>No customer requests recorded today.</Text>
            )}
            <View style={styles.rowSubtotal}>
        ...
        {report.notes.length ? (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>6. STORE OBSERVATIONS</Text>
            <View style={styles.card}>
              {report.notes.map((note, index) => (
                <View key={`${note.storeName}-${index}`} style={styles.noteBlock}>
                  <Text style={styles.noteMeta}>{note.storeName}</Text>
        ...
        <View style={styles.footer} fixed>
          <Text>{report.group.name} — {fullDate(report.businessDate)}</Text>
